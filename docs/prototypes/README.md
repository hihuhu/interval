...
1. `admin-account-management.html` — 管理员账号管理与忘记密码流程原型。
2. `modern-time-grid-v2.html` — 当前时间格主原型。
3. `stats-category-duration.html` — 分类耗时统计页面原型。
4. `login.html` — 早期登录页面原型，仅作登录视觉参考。
...
**状态**：管理员账号管理与忘记密码流程原型  
**用途**：验证默认管理员、首次改密、忘记密码提示、用户列表、活跃度仪表盘、重置密码、禁用/启用账号等管理体验。

**主要功能**：

- 登录页展示普通用户与管理员登录分流。
- 系统无管理员时默认 `admin / 123456`，管理员首次登录必须修改密码。
- 忘记密码页提示联系管理员重置密码。
- 管理后台仅面向管理员账号，不包含时间格和统计入口。
- 仪表盘展示总用户、启用账号、禁用账号、今日活跃、近 7 日活跃。
- 用户列表支持用户名搜索、状态筛选、查看最近登录/最近活跃/近 7 日记录数。
- 支持重置密码、展示临时密码、标记用户下次登录必须改密。
- 支持禁用/启用账号。
- 本地模拟数据，不连接真实 API。

**查看方法**：

1. 双击打开 `admin-account-management.html`。
2. 使用顶部状态切换查看“登录 / 忘记密码 / 管理员首次改密 / 管理后台”。
3. 在登录页使用 `admin / 123456` 演示首次改密流程。
4. 在管理后台点击用户行、重置密码、禁用/启用账号观察交互状态。

**设计文档**：

- `../design/sdd-admin-account-management.md`
