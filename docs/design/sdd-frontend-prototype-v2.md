...
## 12. 2026-05 视觉升级与混合多选规则

### 12.1 原型视觉方向

`docs/prototypes/modern-time-grid-v2.html` 已升级为当前时间格主参考原型。新版原型继续使用 Vue 3 CDN + Tailwind CDN 的单文件形式，但视觉方向从早期轻量卡片演示升级为更接近正式产品的桌面工作台：

- 使用浅色工作台背景、玻璃感面板、柔和边框和统一阴影层级。
- 顶部增加产品化概览区，用于展示当天记录量、完成度、分类数和当前选择。
- 时间网格保留 96 个 15 分钟块的核心结构，强化 hover、dragging、selected 三种状态。
- 右侧面板分为“当前选择编辑”和“分类管理”两块，减少弹窗打断。
- 统计页原型 `docs/prototypes/stats-category-duration.html` 与时间格原型共用同一套视觉语言。

### 12.2 右侧面板混合多选状态

多选可能同时包含空白块、已登记块、不同分类块、不同备注块。右侧面板不得沿用旧的 `modalCat` 或 `modalNote` 造成误导，必须根据当前选择实时计算显示状态。

#### 分类回显规则

| 当前选择 | 右侧分类表现 | 保存行为 |
| --- | --- | --- |
| 全部未登记 | 不预选分类 | 用户必须选择分类后才能保存 |
| 已登记块均为同一分类 | 自动回显该分类 | 保存时可覆盖为当前分类 |
| 已登记块包含多个分类 | 不预选分类，并提示“当前选择包含多个分类” | 用户必须重新指定统一分类 |
| 空白块 + 单一已登记分类 | 自动回显已登记分类 | 保存时会把空白块也写入该分类 |
| 空白块 + 多个已登记分类 | 不预选分类 | 用户必须重新指定统一分类 |

#### 备注回显规则

| 当前选择 | 右侧备注表现 | 保存行为 |
| --- | --- | --- |
| 全部无备注 | 文本框为空 | 未主动编辑时不写备注 |
| 所有备注完全相同 | 自动回显该备注 | 主动修改后批量覆盖 |
| 备注不完全相同 | 文本框为空，并提示“当前选择包含不同备注” | 未主动编辑时保留各块原备注 |

备注保存需要区分“未编辑”和“主动清空”：

- `noteTouched = false`：用户没有动过备注输入框，只更新分类，保留每个块已有备注。
- `noteTouched = true` 且备注非空：所有选中块统一写入该备注。
- `noteTouched = true` 且备注为空：删除所有选中块备注。

### 12.3 状态计算建议

正式 Vue 实现建议从当前选择集派生以下状态，而不是直接依赖上一次表单值：

```typescript
type SelectionCategoryMode = 'empty' | 'single' | 'mixed';
type SelectionNoteMode = 'empty' | 'single' | 'mixed';
...
1. 无选择：清空分类、备注和 `noteTouched`。
2. 单一分类：回显分类；混合分类：清空分类。
3. 单一备注：回显备注；混合备注：清空备注但保留各块原备注。
4. 用户编辑备注后，将 `noteTouched` 标记为 `true`。

### 12.4 验收补充

- 多选不同分类的已登记块时，右侧不得显示旧分类。
- 多选不同备注的已登记块时，右侧不得显示旧备注。
- 混合备注状态下直接保存分类，不得清空或覆盖原备注。
- 主动清空备注并保存时，才删除选中块备注。
- 从混合选择切换回单一分类 / 单一备注选择时，右侧应重新正确回显。
